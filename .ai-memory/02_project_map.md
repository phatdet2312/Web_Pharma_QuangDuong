...
| Static assets | `src/main/resources/static` | CSS/JS/images | JS: `security-core.js`, `auth-sync.js`, `bootstrap.min.js`, `jquery-3.7.0.min.js`, `page-transition-manager.js` (IIFE chống nháy trang), `rich-content-editor.js` (Rich Content Editor WYSIWYG 1017 dòng); CSS: `design-system.css`, `user.css`, `admin.css`, `checkout.css`, `checkout-result.css`, `order-history.css`, `bootstrap.min.css`, `page-transitions.css` (skeleton + fade prefix ptm-), `rich-content-editor.css` (editor CSS prefix rce- 196 dòng) |
