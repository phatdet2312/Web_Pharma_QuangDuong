# Hook: PreToolUse / PermissionRequest for Bash - block dangerous commands.
        ...
        if ($hookEvent -eq 'PermissionRequest') {
            $resp = @{
                hookSpecificOutput = @{
                    ...
                    decision = @{
                        behavior = 'deny'
                        message  = "Dangerous command blocked: $pat"
                    }
                }
            ...
            $resp = @{
                hookSpecificOutput = @{
                    ...
                    permissionDecisionReason = "Dangerous command blocked: $pat"
                }
            } | ConvertTo-Json -Compress -Depth 6
        }
        Write-Output $resp
