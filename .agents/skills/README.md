# .agents/skills/ — Workflow tái sử dụng (on-demand)

## Mục đích
Skills là workflow có hệ thống, CHỈ load khi được gọi hoặc auto-detect.
Không tốn token khi không dùng — khác với rules (load theo path) và AGENTS.md (load mọi phiên).
...
├── review-module/SKILL.md         ← $review-module — Two-Tier Review: tầng 1 (medium) → escalate tầng 2 (high) deep-reviewer/security-auditor
...
├── detect-drift/SKILL.md          ← $detect-drift — phát hiện thay đổi ngoài Codex
...
├── agent-roi/SKILL.md             ← $agent-roi — phân tích chi phí + ROI mỗi subagent (telemetry CSV)
...
| Skill | Invoke | Trigger tự động khi |
|-------|--------|---------------------|
...
1. **Auto-detect**: Codex đọc `description` trong SKILL.md → nếu khớp yêu cầu user → tự load
2. **Thủ công**: User gõ `$tên-skill` hoặc `/skills` selector
3. **Từ subagent khác**: orchestrator có thể trigger skill cho subagent

## So sánh Skills vs Rules vs AGENTS.md

| Cơ chế | Load khi nào | Tốn token khi không dùng? | Ai quản lý |
|--------|-------------|---------------------------|------------|
| AGENTS.md (root + nested cascade) | Mọi phiên (theo cwd) | Có (luôn load) | Developer |
| `.codex/rules/` chi tiết | On-demand khi nested AGENTS.md trỏ tới | Không | Developer |
| Skills | Khi invoke/auto-detect | KHÔNG | Developer |

## Tùy chỉnh

- Thêm skill mới: tạo thư mục + `SKILL.md` trong `.agents/skills/`
...
Subagent sẽ tự điều chỉnh theo convention trong `01_system_architecture.md` (bootstrap tự điền).
Nếu muốn tùy chỉnh sâu hơn, sửa nội dung bên trong SKILL.md cho khớp stack thực tế.
`self-eval/` đã liệt kê build/test command cho nhiều ngôn ngữ — subagent chọn đúng lệnh từ memory.
